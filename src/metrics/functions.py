...
#                 #print(place)
#                 #print(num_objetos)
#                 k_i = len(np.where(recomend_usuario_i == -1)[0]) #+ enlaces_borrados_de_i
#                 #print(np.where(recomend_usuario_i == -1))
#                 #print(k_i)
...
def recovery(matriz_recomendaciones, enlaces_borrados):
    ...
    num_objetos = matriz_recomendaciones.shape[1]
    num_usuarios_con_enlace_borrados = 0
    objetos_borrados_usuarios = [[] for i in repeat(None, int(num_usuarios))]
    for (usuario, objetos) in enlaces_borrados:
        objetos_borrados_usuarios[int(usuario)].append(objetos)
    for i in range(len(matriz_recomendaciones)):
        if i != 1613:
            r_i = []
            #print(i)
            recomend_usuario_i = matriz_recomendaciones[i] #esta es la tira de recomendaciones del usuario i
            objetos_borrados_usuario_i = objetos_borrados_usuarios[i]
            enlaces_borrados_de_i = len(objetos_borrados_usuario_i) 
            if enlaces_borrados_de_i != 0:
                num_usuarios_con_enlace_borrados += 1
                for objetos in objetos_borrados_usuario_i:
                    print(i)
                    print(objetos)
                    print(np.where(recomend_usuario_i == objetos))
                    place = 1 + np.where(recomend_usuario_i == objetos)[0][0] #posicion en la q fue recomendada la peli
                    #print(place)
                    #print(num_objetos)
                    k_i = len(np.where(recomend_usuario_i == -1)[0]) #+ enlaces_borrados_de_i
...
#     p = []
#     r = []
#     n_usuarios, n_objetos = matriz_recomendaciones.shape 
#     for i in range(len(matriz_recomendaciones)):
#         d_i = 0
...
def precision_and_recall(matriz_recomendaciones, enlaces_borrados, L): #L es el largo de la lista de recomendacion a considerar
    ...
    for (usuario, objetos) in enlaces_borrados:
        objetos_borrados_usuarios[int(usuario)].append(objetos)
    for i in range(len(matriz_recomendaciones)):
        recomend_usuario_i = matriz_recomendaciones[i][0:L]
        d_i = sum(np.in1d(recomend_usuario_i, objetos_borrados_usuarios[i]))
        enlaces_borrados_de_i = len(objetos_borrados_usuarios[i])
        if enlaces_borrados_de_i != 0:
            p.append(d_i/L)
            r.append(d_i/enlaces_borrados_de_i)
    return np.mean(p), np.mean(r), np.mean(p)*num_usuarios*num_objetos/len(enlaces_borrados), np.mean(r)*num_objetos/L #los ultimos dos devuelven comparando contra un modelo nulo
...
def personalization(matriz_recomendaciones, L):
    n_users, n_movies = matriz_recomendaciones.shape 
    adyacencia_recomendaciones = np.zeros((n_users, n_movies))
    recomendaciones_L = matriz_recomendaciones[:, :L]-n_users
    for i in range(n_users):
        adyacencia_recomendaciones[i][recomendaciones_L[i].astype(int)] = 1
    q_matrix = np.matmul(adyacencia_recomendaciones, adyacencia_recomendaciones.T)
    h_matrix = 1 - q_matrix/L  
    h_matrix = np.triu(h_matrix, k=0)
    return np.sum(h_matrix)*2/(n_users*(n_users-1))
